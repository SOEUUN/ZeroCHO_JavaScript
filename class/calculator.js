  ...
  const onClickNumber = (number) => (event) => {
    ...
    // 화살표 함수는 중괄호와 return이 있으면 생략 가능함
  };  // 고차 함수(high order function / 함수가 함수를 리턴 하는 것)

  // 함수에서 중복이 발생 했을 경우 (중복 되는 부분을 찾아서 매개변수로 빼기)

  document.querySelector('#num-0').addEventListener('click', onClickNumber('0')); //('click', 함수자리(onClickNumber의 return자리))
